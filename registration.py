...
# CONFIGURATION
# ==========================================
SERVICE_ACCOUNT_FILE = "studentregistration-496003-43196ec7a3c9.json"
GOOGLE_SHEET_ID = "19HXQUVnFQpI3dW9_7pP-Mnj_ESSa0sxJOAoY0NYzKwU"
GOOGLE_SHEET_NAME = "Student Information"
DATA_TAB_NAME = "Pilot Group Fall 2026"
DATA_TAB_GID = 1537140580
SUBMISSION_TAB_NAME = "Submissions"
...
def load_data():
    ...
    try:
        sheet = spreadsheet.worksheet(DATA_TAB_NAME)
    except Exception:
        sheet = spreadsheet.get_worksheet_by_id(DATA_TAB_GID)

    df = pd.DataFrame(sheet.get_all_records())
    df.columns = df.columns.astype(str).str.strip()

    if "StudentID" not in df.columns or "C#" not in df.columns:
        raise ValueError("Google Sheet must contain StudentID and C# columns.")
...
def get_or_create_submission_sheet():
    spreadsheet = get_spreadsheet()
    try:
        return spreadsheet.worksheet(SUBMISSION_TAB_NAME)
    except Exception:
        return spreadsheet.add_worksheet(title=SUBMISSION_TAB_NAME, rows=1000, cols=30)

def append_submission_to_google_sheet(submission_data):
    sheet = get_or_create_submission_sheet()
    headers = [
        "SubmittedAt", "ConfirmationNumber", "Status", "StudentID", "FullName", "CNumber",
        "DHS Email", "Personal Email", "Phone", "Address", "Semester", "Courses", "Tuition",
        "FinancialAid", "FAFSA", "PaymentElection", "RequiredDocumentsUploaded",
        "OptionalDocumentsUploaded", "DocumentFilesJSON"
    ]
    ...
    if not existing:
        ...
        if len(current_headers) == 0 or current_headers[0] != "SubmittedAt":
            sheet.insert_row(headers, 1)

    row = [submission_data.get(h, "") for h in headers]
...
# ==========================================
    ...
    with col_submit:
        if st.button("Submit Registration ✔", disabled=not declaration, type="primary"):
            ...
            try:
                append_submission_to_google_sheet(submission)
                st.session_state.confirmation_number = confirmation
                st.session_state.submitted = True
                st.session_state.step = 8
                st.rerun()
            except Exception as e:
                st.error(f"Submission could not be saved to Google Sheet: {e}")
